...
% Last Modified by GUIDE v2.5 06-Jan-2017 13:49:48
...
book_hedger.arrange_hedge_target(target_value);
% display_deltas
display_deltas(current_delta, delta_value, target_value);
% 更新表格
...
book_hedger.arrange_hedge_target(target_value);
% display_deltas
display_deltas(current_value, delta_value, target_value);
% 更新表格
display_hedgers_table;
...
% display_deltas
display_deltas(current_delta, delta_delta, target_delta);
...
global book_hedger;
book_hedger.hedgers_.removeByIndex(selected_row_index);
book_hedger.check_hedge_condition;
current_value = book_hedger.getRiskDelta;
delta_value = book_hedger.getHedgeDelta;
...
function display_deltas(current_delta, delta_delta, target_delta)
%function display_deltas(current_delta, delta_delta, target_delta)
...
% 更新表格
global book_hedger;
target_delta = str2num(get(findobj('tag', 'target_value_edit'), 'String'));
if isnan(target_delta)
    target_delta = 0;
    set(findobj('tag', 'target_value_edit'), 'String', num2str(target_delta));
end
book_hedger.check_and_prepare_hedge(target_delta);
current_delta = book_hedger.getRiskDelta;
delta_delta = target_delta - current_delta;
display_deltas(current_delta, delta_delta, target_delta);
display_hedgers_table;

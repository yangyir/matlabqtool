...
L = pos.latest;
k_array = zeros(size(pos));
for k_loop = 1:L
    k_array(k_loop) = pos.node(k_loop).parseKFromName;
end
min_k = min(k_array);
max_k = max(k_array);
x = min_k - 0.5 : 0.05: max_k + 0.5;
...
hold on;
% ToDo: vol should be sum of volumes at K.
% for display issue, we should show a txt of actual volume. 

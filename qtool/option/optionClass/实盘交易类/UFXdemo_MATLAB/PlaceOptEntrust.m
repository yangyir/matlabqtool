function [errorCode, errorMsg,entrustNo] = PlaceOptEntrust(connection,token,combiNo,marketNo,stockCode,entrustDirection,futureDirection, entrustPrice,entrustAmount, coveredFlag)
    import com.hundsun.esb.*  com.hundsun.esb.data.*; 
    [errorCode, errorMsg,packet] = CallService(connection,MakeRequestPacket(token,combiNo,marketNo,stockCode,entrustDirection, futureDirection,entrustPrice,entrustAmount, coveredFlag)); 
    entrustNo = -1;
    if errorCode == 0
...
function [message] = MakeRequestPacket(token,combiNo,marketNo,stockCode,entrustDirection, futureDirection, entrustPrice,entrustAmount, coveredFlag)
    import com.hundsun.esb.data.* com.hundsun.esb.impl.*;
    message = javaObject('com.hundsun.esb.data.EsbMessage');
    %委托功能号：91005
    message.prepare(Tagdef.REQUEST_PACKET,'91005');

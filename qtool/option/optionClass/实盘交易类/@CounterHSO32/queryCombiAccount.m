function [errorCode, errorMsg, packet] = queryCombiAccount(self)
connection  = self.connection;
token       = self.token;
accountCode = self.accountCode;
combiNo     = self.combiNo;

[errorCode, errorMsg, packet] = QueryCombi(connection, token, accountCode, combiNo);

end

function [errorCode, errorMsg,packet] = QueryCombi(connection,token,accountCode,combiNo)
...
function [message] = MakeRequestPacket(token,accountCode,combiNo)
    ...
    %账户查询功能号：34001
    message.prepare(Tagdef.REQUEST_PACKET,'35011');  

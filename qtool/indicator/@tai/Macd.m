...
if type ==1
    ...
    sig_long(crossOver(barVal,zeroline)) = 1;
    sig_short(crossOver(zeroline, barVal)) = -1;
    sig_rs(barVal > zeroline) = 1;
    sig_rs(barVal < zeroline) = -1;
elseif type ==2;
    zeroline = zeros(nPeriod, nAsset);
    sig_long(crossOver(barVal,zeroline) & crossOver(diffVal,zeroline) & crossOver(daeVal,zeroline)) = 1;
    sig_short(crossOver(zeroline,barVal) & crossOver(zeroline,diffVal) & crossOver(zeroline,daeVal)) = 1;

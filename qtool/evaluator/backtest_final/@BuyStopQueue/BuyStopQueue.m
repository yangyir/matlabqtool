classdef BuyStopQueue<handle
    %% 买入止损单队列。
    % 存放买入止损单。交易信号发出后，必须指明限定价格，然后入队。
    % 单子按照限定价格由低到高排列。
    % 单子出队以后，应加时间戳然后进入指令缓存序列（OrderQueue）。
    ...
    methods
        ...
        function add(obj,newOrder)
            ...
            % 如果是空队列，直接插入；否则，将新单子插入到限定价格小于其
            % 限定价格的最后一个单子之后。如果限定价格小于第一个单子，插入
            % 到队首。

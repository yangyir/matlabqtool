...
if exist('levels','var') == 0
    % SELECT TOP 10 * FROM dbo.TableImformation WHERE SecID = 'IF1312'　AND [Date] = 20131201
...
sql_select =  ['select * from TFQuant.',codeFName, ' where [Time] >= ',num2str(reqDay),' and [Time] < ',num2str(reqDay+1), ' order by [Time]'];
...
ts.last     = tickData(:,3);
if strcmp(type , '2') && levels == 1
    ts.openInt  = tickData(:,6);
    ...
    ts.askP     = tickData(:,9);
    ts.askV     = tickData(:,10);
elseif strcmp(type , '2') && levels == 10
    ts.openInt  = tickData(:,6);
    ts.volume   = tickData(:,4);
    ts.amount   = tickData(:,5);
    ts.bidP     = tickData(:,[7,11,15,19,23]);
    ts.bidV     = tickData(:,[8,12,16,20,24]);
    ts.askP     = tickData(:,[9,13,17,21,25]);
    ts.askV     = tickData(:,[10,14,18,22,26]);
